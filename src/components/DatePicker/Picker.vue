...
export default {
  ...
  methods: {
    ...
    mountPicker(picker) {
      this.picker = picker || new Vue(this.panel).$mount();
      ...
      this.picker.$on('show', this.onShow);
      this.picker.$on('hide', this.onHide);
    },
    unmountPicker() {
      if (this.picker) {
        this.picker.$destroy();
    ...
    onShow() {
      ...
      let start = wrap.querySelector('td.start-date');
      let top = 999999999;
      setTimeout(() => {
        if (start) {
          top = parent(start, 'month-item').offsetTop;
        }
        wrap.scrollTo(0, top);
      }, 300);
    },
...
.el-input {
  &.el-autocomplete.el-date-editor {
    width: 300px;
  }
}
