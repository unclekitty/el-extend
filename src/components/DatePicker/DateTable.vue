...
export default {
  ...
  methods: {
    ...
    markRange(maxDate) {
      ...
      // range length
      let start = clearHours(maxDate);
      let end = clearHours(minDate);
      this.rangeLength = this.houers(start, end);
      if (this.rangeLength > this.maxRange) {
        return;
      }
      //
        ...
        if (hasClass(target, 'available')) {
          if (this.minDate && this.maxDate) {
            ...
            if (newDate >= this.minDate) {
              ...
              let max = clearHours(maxDate);
              let min = clearHours(this.minDate);
              if (this.houers(max, min) <= this.maxRange) {
                this.$emit(
                  'pick',
                  {
                    minDate: this.minDate,
              ...
              const minDate = new Date(newDate.getTime());
              let max = clearHours(minDate);
              let min = clearHours(this.maxDate);
              if (this.houers(max, min) <= this.maxRange) {
                this.$emit(
                  'pick',
                  {
    ...
    houers(start, end) {
      return Math.abs((start - end) / (24 * 60 * 60 * 1000));
    }
  }
};
</script>
